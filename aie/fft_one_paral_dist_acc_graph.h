...
private:
  xf::dsp::aie::fft::dit_1ch::fft_ifft_dit_1ch_graph<
      ...
      >
      fft_kernel[4];
  adf::kernel wdt_dist_io_strm_kernel[4];
  adf::kernel wdt_coll_io_strm_kernel[4];
...
public:
  ...
  FftOneParalDistAccGraph() {
    ...
    for (unsigned int i = 0; i < 3; i++) {
      //   wdt_dist_io_strm_kernel[i] = adf::kernel::create(wdt_dist_io_strm<3 -
      //   i>);
      source(wdt_dist_io_strm_kernel[i]) = "./widget_distributer.cpp";
      runtime<ratio>(wdt_dist_io_strm_kernel[i]) = 0.8;
    }
    for (unsigned int i = 0; i < 3; i++) {
      //   wdt_coll_io_strm_kernel[i] = adf::kernel::create(wdt_coll_io_strm<3 -
      //   i>);
    ...
    adf::connect<>(wdt_dist_io_strm_kernel[2].out[0], fft_kernel[3].in[0]);
    adf::connect<>(wdt_dist_io_strm_kernel[2].out[1], fft_kernel[2].in[0]);

    adf::connect<>(fft_kernel[3].out[0], wdt_coll_io_strm_kernel[2].in[0]);
    adf::connect<>(fft_kernel[2].out[0], wdt_coll_io_strm_kernel[2].in[1]);
    for (int i = 0; i < 2; i++) {
